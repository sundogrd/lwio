...
//     out of view.
export function menuBar (options: any) {
  return new Plugin({
    view (editorView: any) { return new MenuBarView(editorView, options) }
  })
}
...
export class MenuBarView {
  ...
  maxHeight: any;
  widthForMaxHeight: any;
  floating: any;

  contentUpdate: any;

  scrollFunc: any;
  constructor (editorView: any, options: any) {
    ...
    if (options.floating && !isIOS()) {
      ...
      this.scrollFunc = (e: MouseEvent) => {
        let root = this.editorView.root
        if (!(root.body || root).contains(this.wrapper)) {
          potentialScrollers.forEach(el => el.removeEventListener('scroll', this.scrollFunc))
        } else {
          this.updateFloat((e.target! as any).getBoundingClientRect && e.target)
        }
      }
  ...
  updateFloat (scrollAncestor?: any) {
    let parent = this.wrapper; let editorRect = parent.getBoundingClientRect()
    let top = scrollAncestor ? Math.max(0, scrollAncestor.getBoundingClientRect().top) : 0
...
function getAllWrapping (node: any) {
  let res = [window]
