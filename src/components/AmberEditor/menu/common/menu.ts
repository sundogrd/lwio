...
function translate (view: any, text: any) {
...
function markMenuEvent (e: any) {
  lastMenuEvent.time = Date.now()
  lastMenuEvent.node = e.target
}
function isMenuEvent (wrapper: any) {
...
function renderDropdownItems (items: any, view: any) {
  let rendered = []; let updates = []
...
function combineUpdates (updates: any, nodes: any) {
  return (state: any) => {
    let something = false
...
// be empty).
export function renderGrouped (view: any, content: any) {
  let result = document.createDocumentFragment()
  let updates: any = []
  let separators: any = []
  for (let i = 0; i < content.length; i++) {
    let items = content[i]; let localUpdates = []; let localNodes = []
...
export const selectParentNodeItem = new MenuItem({
  ...
  select: (state: any) => selectParentNode(state),
...
export function wrapItem (nodeType: any, options: any) {
...
export function blockTypeItem (nodeType: any, options: any) {
